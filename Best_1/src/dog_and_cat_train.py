...
#train_file = os.path.join(local_data_path)
...
# from keras.applications.vgg16 import VGG16
# from keras.models import Model
# from tensorflow.keras.utils import to_categorical  # 更新的导入
#
# import tensorflow as tf
# from moxing.framework import file
# import logging
...
# from keras import backend as K
#
# # 配置日志记录器的基本设置，将日志级别设置为INFO，以便在执行过程中记录重要信息
# logging.basicConfig(level=logging.INFO)
#
# # 设置Keras中图像数据的格式为'channels_last'，即通道在最后，这是大多数硬件和模型所采用的格式
# K.set_image_data_format('channels_last')
#
# tf.flags.DEFINE_integer('max_epochs', 10, 'number of training iterations.')
# tf.flags.DEFINE_string('data_url', '/cache/data', 'dataset directory.')
...
#         labels.append(0)
#
# labels = to_categorical(labels, num_classes)  # 使用 to_categorical
# train_data, test_data, train_label, test_label = train_test_split(images_data, labels, test_size=0.25, random_state=10)
#
# optimizer = RMSprop(lr=learning_rate)
# objective = 'binary_crossentropy'
