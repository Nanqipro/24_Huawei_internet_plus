...
class VideoReader():
    ...
    def get_capInfo(self):
        ...
        self.fps = int(self.cap.get(cv2.CAP_PROP_FPS))  # 取整
        self.video_frame_count = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT))
        print('height:', self.height, 'width:', self.width, 'fps:', self.fps, 'video_frame_count:', self.video_frame_count)
        return self.height, self.width, self.fps, self.video_frame_count
